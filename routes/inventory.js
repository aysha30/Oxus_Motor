...
router.get('/advance-search/basic/:company/:model/:trim/:year/:lprice/:hprice/:lmileage/:hmileage/:num_cyliners/:cylinders/:condition/:num', async(req, res) => {
    try{
        ...
        if(num == 1)
            ...
            car = await Car.find({$and:[{'company': company}, {'model': model}, {'trim': trim}, {'year': year}, {'price': {$gte: lprice, $lte: hprice}}, {'mileage': {$gte: lmileage, $lte: hmileage}}, {'num_cyliners': num_cyliners}, {'cylinders': cylinders}, {'condition': condition}]}).sort({price: 1}).limit(2)
        else
            car = await Car.find({$and:[{'company': company}, {'model': model}, {'trim': trim}, {'year': year}, {'price': {$gte: lprice, $lte: hprice}}, {'mileage': {$gte: lmileage, $lte: hmileage}}, {'num_cyliners': num_cyliners}, {'cylinders': cylinders}, {'condition': condition}]}).sort({price: -1}).limit(2)
        res.json({
            "data": car,
